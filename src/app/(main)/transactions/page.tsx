...
const expenseCategoriesList: ExistingExpenseCategory[] = ['Staff Salaries', 'Taxes', 'Utilities', 'Supplies', 'Maintenance', 'Marketing', 'Other'];
...
// Form states
interface FormCashSale extends Omit<CashSale, 'id' | 'date' | 'currency'> {
  date: Date;
  currency: Currency;
 ...
 useEffect(() => {
    ...
    try {
      if (selectedFocus === 'sales') {
        if (selectedSalesType === 'cash_sale') {
          ...
          const cashRecordToSave: Omit<CashSale, 'id' | 'created_at' | 'updated_at'> = {
          ...
          if (isShortfall) { 
            ...
            const { error: insertCashError } = await supabase.from('cash_sales').insert([{ ...cashRecordToSave, id: `cash_ps_${Date.now()}` }]);
            if (insertCashError) { toast({ title: `Error recording partial cash payment`, description: insertCashError.message, variant: "destructive" }); setIsSubmitting(false); return; }
            successMessage = `Partial Cash Payment Recorded.`;
            ...
            if (!error) successMessage += ` Credit Sale Created for Balance of ${creditToSave.currency} ${shortfallAmount.toFixed(2)}.`;
          } else {
            const { error: insertCashError } = await supabase.from('cash_sales').insert([{ ...cashRecordToSave, id: `cash_${Date.now()}` }]);
            error = insertCashError;
            if (!error) successMessage = "Cash Sale Recorded";
          }
        ...
        if (selectedPurchaseType === 'cash_purchase') {
            ...
            const expenseToSave: Omit<Expense, 'id' | 'created_at' | 'updated_at'> = {
                ...
                paid_to: cashPurchaseData.vendor_id ? vendorsList.find(v=>v.id === cashPurchaseData.vendor_id)?.name : undefined, // Store name if ID known
  ...
  const renderSalesForm = () => {
    switch (selectedSalesType) {
      case 'cash_sale':
        return (
          <form onSubmit={(e) => { e.preventDefault(); handleSubmit(); }} className="grid gap-4">
            ...
            {isShortfall && (<><div className="grid gap-2 p-3 border border-destructive rounded-md bg-destructive/10"><div className="flex items-center justify-between"><Label htmlFor="cash_customer_name_shortfall" className="text-destructive">Customer Name (for credit)</Label><Button type="button" variant="ghost" size="icon" className="h-7 w-7 text-primary hover:text-primary/80" onClick={() => router.push(`/customers?redirect=${pathname}&newEntityName&newEntityOrigin=transactionShortfall`)} disabled={isSubmitting || isLoading} title="Add New Customer"><PlusCircle className="h-5 w-5" /></Button></div><Select value={cashSaleData.customer_name || '_SELECT_CUST_SF_'} onValueChange={(value) => handleSelectChange('cash_sale', 'customer_name', value)} disabled={isSubmitting || isLoading}><SelectTrigger id="cash_customer_name_shortfall"><SelectValue placeholder={isLoading ? "Loading customers..." : "Select customer"} /></SelectTrigger><SelectContent>{isLoading && <SelectItem value="_LOADING_CUST_SF_" disabled>Loading customers...</SelectItem>}{!isLoading && customersList.length === 0 && <SelectItem value="_EMPTY_CUST_SF_" disabled>No customers found</SelectItem>}<SelectItem value="_SELECT_CUST_SF_" disabled>Select customer</SelectItem>{customersList.map(customer => <SelectItem key={customer.id} value={customer.name}>{customer.name}</SelectItem>)}</SelectContent></Select></div><div className="grid gap-2"><Label htmlFor="due_date_for_balance">Due Date for Balance</Label><Popover><PopoverTrigger asChild><Button variant={"outline"} className={cn("w-full justify-start text-left font-normal", !cashSaleData.due_date_for_balance && "text-muted-foreground")} disabled={isSubmitting}><CalendarIcon className="mr-2 h-4 w-4" />{cashSaleData.due_date_for_balance ? format(cashSaleData.due_date_for_balance, "PPP") : <span>Pick due date</span>}</Button></PopoverTrigger><PopoverContent className="w-auto p-0"><Calendar mode="single" selected={cashSaleData.due_date_for_balance} onSelect={(d) => handleDateChange('cash_sale', 'due_date_for_balance', d)} /></PopoverContent></Popover></div></>)}
            {!isShortfall && (<div className="grid gap-2"><div className="flex items-center justify-between"><Label htmlFor="cash_customer_name_optional">Customer Name (Optional)</Label><Button type="button" variant="ghost" size="icon" className="h-7 w-7 text-primary hover:text-primary/80" onClick={() => router.push(`/customers?redirect=${pathname}&newEntityName&newEntityOrigin=transactionShortfall`)} disabled={isSubmitting || isLoading} title="Add New Customer"><PlusCircle className="h-5 w-5" /></Button></div><Select value={cashSaleData.customer_name || '_SELECT_CUST_OPT_'} onValueChange={(value) => handleSelectChange('cash_sale', 'customer_name', value)} disabled={isSubmitting || isLoading}><SelectTrigger id="cash_customer_name_optional"><SelectValue placeholder={isLoading ? "Loading customers..." : "Select customer (Optional)"} /></SelectTrigger><SelectContent>{isLoading && <SelectItem value="_LOADING_CUST_OPT_" disabled>Loading customers...</SelectItem>}{!isLoading && customersList.length === 0 && <SelectItem value="_EMPTY_CUST_OPT_" disabled>No customers found</SelectItem>}<SelectItem value="_SELECT_CUST_OPT_">None (Optional)</SelectItem>{customersList.map(customer => <SelectItem key={customer.id} value={customer.name}>{customer.name}</SelectItem>)}</SelectContent></Select></div>)}
            {changeDue !== null && changeDue >= 0 && !isShortfall && (<p className="text-sm font-medium text-green-600">Change Due: {cashSaleData.currency} {(changeDue).toFixed(2)}</p>)}
            {changeDue !== null && changeDue < 0 && !isShortfall && (<p className="text-sm font-medium text-destructive">Amount Tendered is less than Sale Amount. Select customer and set due date to record balance.</p>)}
      ...
      case 'credit_sale':
        return (
          <form onSubmit={(e) => { e.preventDefault(); handleSubmit(); }} className="grid gap-4">
             <div className="grid gap-2"><div className="flex items-center justify-between"><Label htmlFor="credit_customer_name">Customer Name</Label><Button type="button" variant="ghost" size="icon" className="h-7 w-7 text-primary hover:text-primary/80" onClick={() => router.push(`/customers?redirect=${pathname}&newEntityName&newEntityOrigin=transactionCreditSale`)} disabled={isSubmitting || isLoading} title="Add New Customer"><PlusCircle className="h-5 w-5" /></Button></div><Select value={creditSaleData.customer_name || '_SELECT_CUST_CREDIT_'} onValueChange={(value) => handleSelectChange('credit_sale', 'customer_name', value)} disabled={isSubmitting || isLoading}><SelectTrigger id="credit_customer_name"><SelectValue placeholder={isLoading ? "Loading customers..." : "Select customer"} /></SelectTrigger><SelectContent>{isLoading && <SelectItem value="_LOADING_CUST_CREDIT_" disabled>Loading customers...</SelectItem>}{!isLoading && customersList.length === 0 && <SelectItem value="_EMPTY_CUST_CREDIT_" disabled>No customers found</SelectItem>}<SelectItem value="_SELECT_CUST_CREDIT_" disabled>Select customer</SelectItem>{customersList.map(customer => <SelectItem key={customer.id} value={customer.name}>{customer.name}</SelectItem>)}</SelectContent></Select></div>
            <div className="grid gap-2"><Label htmlFor="credit_revenue_category">Revenue Category</Label><Select value={creditSaleData.revenue_category || ''} onValueChange={(value) => handleSelectChange('credit_sale', 'revenue_category', value)} disabled={isSubmitting}><SelectTrigger id="credit_revenue_category"><SelectValue placeholder="Select revenue category" /></SelectTrigger><SelectContent>{revenueCategories.map(cat => <SelectItem key={cat} value={cat}>{cat}</SelectItem>)}</SelectContent></Select></div>
            <div className="grid gap-2"><Label htmlFor="credit_item_service">Item/Service Sold</Label><Input id="credit_item_service" name="item_service" value={creditSaleData.item_service || ''} onChange={(e) => handleInputChange('credit_sale', e)} required disabled={isSubmitting}/></div>
            <div className="grid gap-2"><Label htmlFor="credit_details">Details (Optional)</Label><Textarea id="credit_details" name="details" value={creditSaleData.details || ''} onChange={(e) => handleInputChange('credit_sale', e)} disabled={isSubmitting}/></div>
            <div className="grid grid-cols-3 gap-4"> <div className="grid gap-2"><Label htmlFor="credit_original_amount">Amount</Label><Input id="credit_original_amount" name="original_amount" type="number" value={creditSaleData.original_amount || ''} onChange={(e) => handleInputChange('credit_sale', e)} required disabled={isSubmitting} step="0.01"/></div><div className="grid gap-2"><Label htmlFor="credit_currency">Currency</Label><Select value={creditSaleData.currency || 'USD'} onValueChange={(value) => handleSelectChange('credit_sale', 'currency', value as Currency)} disabled={isSubmitting}><SelectTrigger><SelectValue placeholder="Select currency" /></SelectTrigger><SelectContent><SelectItem value="USD">USD</SelectItem><SelectItem value="SSP">SSP</SelectItem></SelectContent></Select></div><div className="grid gap-2"><Label htmlFor="credit_status">Status</Label><Select value={creditSaleData.status || 'Pending'} onValueChange={(value) => handleSelectChange('credit_sale', 'status', value as CreditStatus)} disabled={isSubmitting}><SelectTrigger><SelectValue placeholder="Select status" /></SelectTrigger><SelectContent><SelectItem value="Pending">Pending</SelectItem><SelectItem value="Paid" disabled>Paid (Set via Payment)</SelectItem><SelectItem value="Overdue">Overdue</SelectItem></SelectContent></Select></div></div>
            <div className="grid grid-cols-2 gap-4"><div className="grid gap-2"><Label htmlFor="credit_date">Transaction Date</Label><Popover><PopoverTrigger asChild><Button variant={"outline"} className={cn("w-full justify-start text-left font-normal", !creditSaleData.date && "text-muted-foreground")} disabled={isSubmitting}><CalendarIcon className="mr-2 h-4 w-4" />{creditSaleData.date ? format(creditSaleData.date, "PPP") : <span>Pick a date</span>}</Button></PopoverTrigger><PopoverContent className="w-auto p-0"><Calendar mode="single" selected={creditSaleData.date} onSelect={(d) => handleDateChange('credit_sale','date',d)} initialFocus /></PopoverContent></Popover></div><div className="grid gap-2"><Label htmlFor="credit_due_date">Due Date (Optional)</Label><Popover><PopoverTrigger asChild><Button variant={"outline"} className={cn("w-full justify-start text-left font-normal", !creditSaleData.due_date && "text-muted-foreground")} disabled={isSubmitting}><CalendarIcon className="mr-2 h-4 w-4" />{creditSaleData.due_date ? format(creditSaleData.due_date, "PPP") : <span>Pick a date</span>}</Button></PopoverTrigger><PopoverContent className="w-auto p-0"><Calendar mode="single" selected={creditSaleData.due_date} onSelect={(d) => handleDateChange('credit_sale','due_date',d)} /></PopoverContent></Popover></div></div>
      ...
      default:
        return <p className="text-muted-foreground text-center py-8">Please select a sales transaction type to begin.</p>;
    }
  };
  ...
  const renderPurchaseForm = () => {
    switch(selectedPurchaseType) {
        ...
        default:
             return <p className="text-muted-foreground text-center py-8">Please select a purchase transaction type to begin.</p>;
    }
  }
